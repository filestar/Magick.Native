...
verifyNative() {
    ...
    # This assumes that Xcode is installed, the path for Commandline tools is different
    sdk_link="-L$(xcode-select -p)/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/usr/lib -lSystem"
    echo ld $file -arch $ld_arch -platform_version macos $macos_version $macos_version -U _main $sdk_link
